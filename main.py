...
print("Gaining list of possible gullible people...")
time.sleep(1)

while 1 < 2:
    a = random.randint(0,100)
    time.sleep(2)
    print(colored("Finding target....", "magenta"))
    def spinning_cursor():
        while True:
            for cursor in '|/-\\':
                yield cursor
    ...
    # time.sleep(0.5)
    print(colored("Retrieving credentials....", "cyan"))
    
    # print("\n")
    time.sleep(random.randint(1, 4))
    x = random.choice(names) + str(random.randint(0,100)) + random.choice(domain)
    print(colored("Found", "blue"), colored(x, "yellow"))
    time.sleep(random.randint(1,4))
    print("Checking possible vulnerabilities")
    def spinning_cursor():
        while True:
            for cursor in '|/-\\':
                yield cursor
    ...
    # time.sleep(1)
    b = ["x", "y", "z"]
    if random.choice(b) == "x":
        print(colored("Failed to retrieve necessary data!", "red"))
        time.sleep(0.5)
        print("Proceeding to the next target....")
        time.sleep(1)
    else:
        print(colored('Successfully retrieved password!', 'green'))
        print(colored("Decrypting...", "red"))
        for i in range(21):
            sys.stdout.write('\r')
            # the exact output you're looking for:
            sys.stdout.write("[%-20s] %d%%" % ('❙'*i, 5*i))
